...
class DataPreprocessor:
    ...
    def normalize_features(self) -> pd.DataFrame:
        ...
        # Max values based on schema constraints
        constraints = {
            'cgpa': 10.0,
    ...
    def build_academic_dataset(self) -> tuple:
        ...
        # Generate target risk based on dummy logic (since labels are assumed locally)
        # In reality, this is tracked historically
        risk = []
    ...
    def build_placement_dataset(self) -> tuple:
        ...
        # Generating synthetic target (1 = Ready, 0 = Not Ready)
        ready = []
        for _, row in df_norm.iterrows():
...
if __name__ == '__main__':
    ...
    X_acad, y_acad = processor.build_academic_dataset()
    X_place, y_place = processor.build_placement_dataset()
    print(f"Academic Data Shape: {X_acad.shape}, {y_acad.shape}")
    print(f"Placement Data Shape: {X_place.shape}, {y_place.shape}")
