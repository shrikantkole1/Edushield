...
class SecureAggregator:
    ...
    def encrypt_weights(self, weights: list):
        ...
        logger.info(f"Encrypted {len(weights)} layer weights into {len(encrypted)} bytes payload.")
        return encrypted
    ...
    def decrypt_aggregated_weights(self, encrypted_payload: bytes):
        ...
        logger.info(f"Decrypted payload successfully. Ready for FedAvg.")
        return weights

if __name__ == '__main__':
    sa = SecureAggregator()
    # Dummy weight tensor list
    dummy_weights = [np.random.rand(10, 10), np.random.rand(10)]
    enc = sa.encrypt_weights(dummy_weights)
    print(f"Encrypted Bytes Format: {enc[:50]}...")
    dec = sa.decrypt_aggregated_weights(enc)
    print(f"Decryption match: {np.array_equal(dummy_weights[0], dec[0])}")
